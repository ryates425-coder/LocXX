...
data class RowLockCelebrationUi(val row: RowId, val rowPoints: Int)
...
private fun rowAccent(row: RowId): Color = when (row) {
    RowId.RED -> Color(0xFFE53935)
    RowId.YELLOW -> Color(0xFFFBC02D)
    RowId.GREEN -> Color(0xFF22C55E)
    RowId.BLUE -> Color(0xFF3B82F6)
}

private fun rowDisplayName(row: RowId): String =
    row.name.lowercase().replaceFirstChar { it.titlecase() }

/**
 * Snotzee-style moment: dim scrim, confetti burst, Snotzee `playBonusHorn` fanfare. Tap or wait to dismiss.
 */
@Composable
fun RowLockCelebrationOverlay(
    celebration: RowLockCelebrationUi?,
    onDismiss: () -> Unit,
    modifier: Modifier = Modifier
) {
    if (celebration == null) return
    val row = celebration.row
    ...
    val accent = rowAccent(row)
    val bits = remember(row) {
        val rng = Random(row.ordinal * 7919)
        List(96) {
            ConfettiBit(
                xFrac = rng.nextFloat(),
                ...
                color = listOf(accent, Color.White, Color(0xFFFFD700), Color(0xFFE91E63))[rng.nextInt(4)],
    ...
    Box(
        ...
        Column(
            ...
            Text(
                text = "You locked the ${rowDisplayName(row)} row!",
                color = accent,
                fontSize = 48.sp,
